	...
	for i in range(1, MAX_ITERS):
		...
		if int(sum(numWeight)) < 1.01: # account for floating point err
			...
			if var_portfolio == std_max:
				...
				EUkeys, EUvalues = EU.keys(), EU.values()
				EWMAkeys, EWMAvalues = EWMA.keys(), EWMA.values()
				profitkeys, profitvalues = profit.keys(), profit.values()
				...
				print(var_portfolio)
				print("  ")
				best[0:4, i] = [sum(list(EUvalues)), var_portfolio, sum(list(EWMAvalues)), sum(list(profitvalues))]
